    ...
    if st.button("**:orange[🎯Modelo, qual é a sua previsão?]**", key="btn_predict_mlp"):
        with st.spinner("Processing..."):
            ...
            if prediction.round()[0] > 0:
                st.success(
                    ":white_check_mark: **Previsão:** . . .  O aluno apresenta alta probabilidade de alcançar o ponto de virada e passar para um nível de desenvolvimento mais elevado. 🏆"
                )
            else:
                st.error(
                    ":x: **Previsão:**. . .  O aluno apresenta baixa probabilidade de alcançar o ponto de virada e passar para um nível de desenvolvimento mais elevado. 😞"
                )
